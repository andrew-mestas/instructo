...
return top}
instructo.prototype.getOccurences=function(name,filter,condition,exact){var subset=this.filterAndChrono(filter,condition,exact,name);subset=subset.filter((x)=>{return x});this.filtered=new instructo(subset).createObject(this.keys);var results=this.depth>=1?this.filtered.dive():this.filtered.results;return this.getCounts(results,this.filtered.array.map((x)=>{return x[name]}).filter(this.onlyUnique))}
...
instructo.prototype.filterAndChrono=function(filter,condition,exact,name){var top=[];var t=[];var count=0;if(filter!==undefined){var filterValues=this.validateFilter(filter);var array=this.array;t=array.filter((x)=>{count=0;var results=filterValues.map((c)=>{if(c.value!==''){count+=1;if(x[c.key]===c.value){return!0}else{return!1}}
return!1});var tally=this.getCounts({r:results},results.map((x)=>{return x}));if(exact){return tally.!1===0}else{return tally.!0===count}});if(condition&&condition.has!==''){var y=new instructo().setArray(t).createObject(this.keys);var groupedItems=y.group(condition);for(var item in groupedItems){var newest=undefined;groupedItems[item].forEach((x)=>{newest=newest===undefined?x[condition.chrono]:(x[condition.chrono]>=newest)?x[condition.chrono]:newest});top.push(groupedItems[item].filter((c)=>{return(c[condition.chrono]===newest&&c[condition.value]===condition.has)}))}
top=top.map((x)=>{return x[0]})
return top}
return t.length===0?this.array:t}else{return this.array}};module.exports=instructo;